...
class BaselineAdaptationTrainer(AcousticModelTrainer):
    """
    ...
    def __init__(self, hparams, num_utts_training):
        ...
        with open(os.path.join(hparams.data_dir, "file_id_list_{}_adapt{}_train.txt"
                                                 .format(hparams.voice, num_utts_training))) as f:
            id_list_train = f.readlines()
        with open(os.path.join(hparams.data_dir, "file_id_list_{}_adapt{}_test.txt"
                                                 .format(hparams.voice, num_utts_training))) as f:
            id_list_test = f.readlines()
        with open(os.path.join(hparams.data_dir, "file_id_list_{}_adapt{}_val.txt"
                                                 .format(hparams.voice, num_utts_training))) as f:
...
def main():
    ...
    hparams.add_hparam("f_get_emb_index", None)  # First add empty parameter.
    ...
    for num_utts_training in [10, 380]:
        ...
        trainer.benchmark(hparams, os.path.join("database", "file_id_list_{}_adapt{}_test_male.txt"
                                                            .format(hparams.voice, num_utts_training)))
        trainer.benchmark(hparams, os.path.join("database", "file_id_list_{}_adapt{}_test_female.txt"
                                                            .format(hparams.voice, num_utts_training)))
